export const dailyQuotes = [
  {
    text: "The secret of getting ahead is getting started.",
    author: "Mark Twain",
  },
  {
    text: "What you do every day matters more than what you do once in a while.",
    author: "Gretchen Rubin",
  },
  {
    text: "Success is the sum of small efforts, repeated day in and day out.",
    author: "Robert Collier",
  },
  {
    text: "Do not wait to strike till the iron is hot; but make it hot by striking.",
    author: "William Butler Yeats",
  },
  {
    text: "The future depends on what you do today.",
    author: "Mahatma Gandhi",
  },
  {
    text: "Well done is better than well said.",
    author: "Benjamin Franklin",
  },
  {
    text: "Small deeds done are better than great deeds planned.",
    author: "Peter Marshall",
  },
  {
    text: "Ooooooo friends",
    author: "William McKenzie",
  },
  {
    text: "Frothing at the gash",
    author: "Jay Cartwright",
  },
  {
    text: "I am not in danger, Skyler. I am the danger. A guy opens his door and gets shot and you think that of me? No. I am the one who knocks!",
    author: "Walter White",
  },
  {
    text: "awobabobob?",
    author: "Neil Sutherland",
  },
  {
    text: "It's that sense of shitting in a bucket in a cupboard you don't get with other holidays… in England… with your parents!",
    author: "William McKenzie",
  },
  {
    text: "I've had 210 wanks and my cock's like a Perperami",
    author: "Simon Cooper",
  },
  {
    text: "Tell you what, that crack is really moreish.",
    author: "Super Hans",
  },
  {
    text: "Mark, if I can just get rid of the dog corpse, there’s a chance I still might get laid here.",
    author: "Jez",
  },
  {
    text: "Four naan, Jeremy? Four? That's insane.",
    author: "Mark Corrigan",
  },
  {
    text: "Hitler promised not to invade Czechoslovakia, Jeremy. Welcome to the real world.",
    author: "Mark Corrigan",
  },
  {
    text: "People like Coldplay and voted for the Nazis. You can't trust people Jeremy.",
    author: "Super Hans",
  },
  {
    text: "Mind out boys, Father Spliffmas coming through.",
    author: "Super Hans",
  },
  {
    text: "You get a van, Jez, we could be men with ven.",
    author: "Super Hans",
  },
  {
    text: "Just open your gob and someone will slip in something tasty. A pill, a nipple, bit of fried halloumi. Lovely.",
    author: "Super Hans",
  },
  {
    text: "Super Hans has taken four grams of coke to 'relax him' for his speech.",
    author: "Jez",
  },
  {
    text: "Butter the toast, eat the toast, shit the toast. God life's relentless.",
    author: "Mark Corrigan",
  },
  {
    text: "I accidentally run to Windsor.",
    author: "Super Hans",
  },
  {
    text: "D'oh!",
    author: "Homer Simpson",
  },
  {
    text: "Speed has never killed anyone. Suddenly becoming stationary, that's what gets you.",
    author: "Jeremy Clarkson",
  },
  {
    text: "No, no, no. There's no such thing as cheap and cheerful. It's cheap and nasty & expensive and cheerful.",
    author: "Jeremy Clarkson",
  },
  {
    text: "Hammond you idiot, you've reversed into the sports lorry",
    author: "Jeremy Clarkson",
  },
  {
    text: "I don't always downshift but when i do it's next to a prius, so they can hear me hurt the environment.",
    author: "Jeremy Clarkson",
  },
  {
    text: "Oh cock",
    author: "James May",
  },
  {
    text: "Every day it gets a little easier. But you gotta do it every day. That's the hard part. But it does get easier.",
    author: "BoJack Horseman",
  },
  {
    text: "Discipline is choosing between what you want now and what you want most.",
    author: "Abraham Lincoln",
  },
  {
    text: "We are what we repeatedly do. Excellence, then, is not an act, but a habit.",
    author: "Will Durant",
  },
  {
    text: "If you're going through hell, keep going.",
    author: "Winston Churchill",
  },
  {
    text: "Action is the foundational key to all success.",
    author: "Pablo Picasso",
  },
  {
    text: "You don’t have to be extreme, just consistent.",
    author: "Unknown",
  },
  {
    text: "Motivation gets you going, but discipline keeps you growing.",
    author: "John C. Maxwell",
  },
  {
    text: "Start where you are. Use what you have. Do what you can.",
    author: "Arthur Ashe",
  },
  {
    text: "The only way to do great work is to love what you do.",
    author: "Steve Jobs",
  },
  {
    text: "Dream big. Start small. Act now.",
    author: "Robin Sharma",
  },
  {
    text: "Hard choices, easy life. Easy choices, hard life.",
    author: "Jerzy Gregorek",
  },
  {
    text: "Done is better than perfect.",
    author: "Sheryl Sandberg",
  },
  {
    text: "Stay hungry. Stay foolish.",
    author: "Steve Jobs",
  },
  {
    text: "Make it happen.",
    author: "Unknown",
  },
  {
    text: "Progress over perfection.",
    author: "Unknown",
  },
  {
    text: "Consistency beats intensity.",
    author: "Unknown",
  },
  {
    text: "Focus. Then execute.",
    author: "Unknown",
  },
  {
    text: "He who has a why to live can bear almost any how.",
    author: "Friedrich Nietzsche",
  },
  {
    text: "The impediment to action advances action. What stands in the way becomes the way.",
    author: "Marcus Aurelius",
  },
  {
    text: "Waste no more time arguing what a good man should be. Be one.",
    author: "Marcus Aurelius",
  },
  {
    text: "Luck is what happens when preparation meets opportunity.",
    author: "Seneca",
  },
  {
    text: "No man ever steps in the same river twice.",
    author: "Heraclitus",
  },
  {
    text: "I choose a lazy person to do a hard job, because a lazy person will find an easy way to do it.",
    author: "Bill Gates",
  },
  {
    text: "I am so clever that sometimes I don't understand a single word of what I am saying.",
    author: "Oscar Wilde",
  },
  {
    text: "If at first you don’t succeed, then skydiving definitely isn’t for you.",
    author: "Steven Wright",
  },
  {
    text: "The trouble with having an open mind is that people will insist on coming along and trying to put things in it.",
    author: "Terry Pratchett",
  },
  {
    text: "I refuse to answer that question on the grounds that I don’t know the answer.",
    author: "Douglas Adams",
  },
  {
    text: "I wish there was a way to know you're in the good old days before you've actually left them.",
    author: "Andy Bernard",
  },
  {
    text: "Bears. Beets. Battlestar Galactica.",
    author: "Jim Halpert",
  },
  {
    text: "I am Beyoncé, always.",
    author: "Michael Scott",
  },
  {
    text: "I'm not superstitious, but I am a little stitious.",
    author: "Michael Scott",
  },
  {
    text: "Never half-ass two things. Whole-ass one thing.",
    author: "Ron Swanson",
  },
  {
    text: "Give 100%. 110% is impossible. Only idiots recommend that.",
    author: "Ron Swanson",
  },
  {
    text: "That’ll do pig. That’ll do.",
    author: "Farmer Hoggett",
  },
  {
    text: "How hard can it be?",
    author: "Jeremy Clarkson",
  },
  {
    text: "You've got red on you.",
    author: "Ed",
  },
  {
    text: "How's that for a slice of fried gold?",
    author: "Ed",
  },
  {
    text: "Go to the Winchester, have a nice cold pint, and wait for all of this to blow over.",
    author: "Shaun",
  },
  {
    text: "You ain't seen Bad Boys II?",
    author: "Danny Butterman",
  },
  {
    text: "It's just the one swan, actually.",
    author: "Danny Butterman",
  },
  {
    text: "You know what's fucked? The amount of times your mum has faked a jelly fish string to get me to piss on her.",
    author: "Shoresy",
  },
  {
    text: "Giggity giggity goo!",
    author: "Glenn Quagmire",
  },
  {
    text: "Shut up, Meg.",
    author: "Peter Griffin",
  },
  {
    text: "Bird is the word.",
    author: "Peter Griffin",
  },
  {
    text: "I'm not a traitor to my class. I am just an extreme example of what a working man can achieve.",
    author: "Thomas Shelby",
  },
  {
    text: "You can change what you do, but you can't change what you want.",
    author: "Thomas Shelby",
  },
];
